...
import com.star.comprehensive.domain.ZodiacSign;
import com.star.comprehensive.domain.User;
import com.star.comprehensive.persistence.FileUserStorage;
import com.star.comprehensive.persistence.UserRepository;
import com.star.comprehensive.service.UserService;
...
public class Application {
    ...
    private void registerUser() {
        try {
            ...
            System.out.print("별자리 입력 (ARIES, TAURUS, GEMINI, CANCER, LEO, VIRGO, LIBRA, SCORPIO, SAGITTARIUS, CAPRICORN, AQUARIUS, PISCES): ");
            ZodiacSign zodiacSign = ZodiacSign.fromString(scanner.nextLine());

            int nextUserNo = userService.findAllUsers().size() + 1;
            User newUser = new User(nextUserNo, id, pwd, age, hobbies, zodiacSign);
    ...
    private void modifyUser() {
        try {
            ...
            System.out.print("새로운 별자리 (" + existingUser.getZodiacSign() + "): ");
            String zodiacInput = scanner.nextLine();
            ZodiacSign zodiacSign = zodiacInput.isEmpty()
                    ? existingUser.getZodiacSign()
                    : ZodiacSign.fromString(zodiacInput);

            User updatedUser = new User(no, id, newPassword, age, hobbies, zodiacSign);
